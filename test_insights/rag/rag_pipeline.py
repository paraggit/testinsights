...
Please provide a clear and helpful response based on the data provided."""
    ...
    def _calculate_metrics(
        ...
        for result in search_results:
            ...
            metrics["by_entity_type"][entity_type] = metrics["by_entity_type"].get(entity_type, 0) + 1
        ...
        if metrics["total_items"] > 0:
            ...
            # Common metrics
            failed_count = (
                metrics["by_status"].get("FAILED", 0) +
                metrics["by_status"].get("BROKEN", 0)
            )
    ...
    async def query_with_feedback(
        ...
        # Add context about this being a refinement
        messages = [
            Message(role="system", content=self.query_processor.build_system_prompt(result["analysis"])),
            Message(role="user", content=f"Original query: {query}"),
            Message(role="assistant", content=previous_response),
            Message(role="user", content=f"This response was incomplete. {feedback} Please provide an improved answer based on the available data."),
        ]
