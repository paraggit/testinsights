...
async def example_with_sources():
    ...
    result = await rag.query(question, n_results=10, include_raw_results=True)
...
async def example_ollama_local():
    ...
    try:
        async with OllamaProvider(base_url="http://localhost:11434", model="llama2") as llm:
            rag = RAGPipeline(llm)
...
async def example_multi_turn_conversation():
    ...
    for question in conversations:
        ...
        context.append({"question": question, "response": result["response"]})
