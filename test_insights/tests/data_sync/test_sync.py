...
def mock_api_client():
    ...
    client.get_projects = AsyncMock(
        return_value=[{"id": 1, "projectName": "test-project", "organization": "test-org"}]
    )
    ...
    client.get_test_items = AsyncMock(
        return_value=PaginatedResponse(items=[], total=0, page=0, size=100)
    )

    client.get_logs = AsyncMock(return_value=PaginatedResponse(items=[], total=0, page=0, size=100))

    client.get_filters = AsyncMock(
        return_value=PaginatedResponse(items=[], total=0, page=0, size=100)
    )

    client.get_dashboards = AsyncMock(
        return_value=PaginatedResponse(items=[], total=0, page=0, size=100)
    )
...
def mock_storage_client():
    ...
    client.get_statistics = AsyncMock(return_value={"total_documents": 0, "by_entity_type": {}})
    return client
...
async def test_sync_orchestrator_full_sync(mock_api_client, mock_storage_client):
    ...
    orchestrator = SyncOrchestrator(api_client=mock_api_client, storage_client=mock_storage_client)

    stats = await orchestrator.sync(
        sync_type="full", project_names=["test-project"], entity_types=["project", "launch"]
    )
...
async def test_sync_orchestrator_incremental_sync(mock_api_client, mock_storage_client):
    ...
    stats = await orchestrator.sync(sync_type="incremental", project_names=["test-project"])
...
async def test_full_sync_strategy(mock_api_client, mock_storage_client):
    ...
    stats = await strategy.sync(project_names=["test-project"], entity_types=["project", "user"])
...
async def test_incremental_sync_strategy(mock_api_client, mock_storage_client):
    ...
    strategy = IncrementalSyncStrategy(mock_api_client, mock_storage_client, lookback_hours=12)
...
def test_paginated_response():
    ...
    response = PaginatedResponse(items=[1, 2, 3], total=10, page=0, size=3)
    ...
    # Test last page
    last_page = PaginatedResponse(items=[10], total=10, page=3, size=3)
...
async def test_sync_error_handling(mock_api_client, mock_storage_client):
    ...
    stats = await orchestrator.sync(sync_type="full", entity_types=["project"])
...
async def test_storage_client_text_extraction():
    ...
    launch_text = client._extract_text_content(
        "launch",
        {
            "name": "Test Launch",
            "description": "Launch description",
            "status": "PASSED",
            "attributes": [{"key": "build", "value": "123"}, {"key": "branch", "value": "main"}],
        },
    )
